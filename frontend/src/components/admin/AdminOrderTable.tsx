                            ...
                            <td>
                                <div style={{ display: 'flex', gap: '4px', flexWrap: 'wrap' }}>
                                    {order.orderStatus === 'PENDING' && (
                                        <button onClick={() => onPlace(order.id)} className="action-btn" style={{ backgroundColor: '#dcfce7', color: '#166534' }} title="Mark Placed">
                                            <CheckCircle size={14} />
                                        </button>
                                    )}
                                    {order.orderStatus === 'PLACED' && (
                                        <button onClick={() => onConfirm(order.id)} className="action-btn" style={{ backgroundColor: '#dbeafe', color: '#1e40af' }} title="Confirm">
                                            <CheckCircle size={14} />
                                        </button>
                                    )}
                                    {order.orderStatus === 'CONFIRMED' && (
                                        <button onClick={() => onShip(order.id)} className="action-btn" style={{ backgroundColor: '#f3e8ff', color: '#6b21a8' }} title="Ship">
                                            <Truck size={14} />
                                        </button>
                                    )}
                                    {order.orderStatus === 'SHIPPED' && (
                                        <button onClick={() => onDeliver(order.id)} className="action-btn" style={{ backgroundColor: '#d1fae5', color: '#065f46' }} title="Deliver">
                                            <Package size={14} />
                                        </button>
                                    )}
